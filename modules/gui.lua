------------------------------------------------------
-- WindowUtils - GUI Module
-- Modern settings layout using library components
------------------------------------------------------
...
-- Section definitions (populated in gui.init after IconGlyphs is available)
gui.sections = {}
...
local function drawGeneralSection(ui)
...
local function drawBackgroundSection(ui)
    local c = controls.bind(settings.master, settings.defaults, settings.save)
    ...
    if changed and blurAvailable then
        ...
        settings.save()
        if settings.master.blurOnOverlayOpen and ui.state.isOverlayOpen
...
local function drawSidebar()
    for _, entry in ipairs(gui.sections) do
        if controls.DynamicButton(entry.label, entry.icon, {
            style = gui.selectedSection == entry.page and "active" or "inactive",
            width = -1,
        }) then
            gui.selectedSection = entry.page
        end
    end
end
...
local function drawContentPanel(ui)
    if controls.BeginFillChild("gui_section_scroll", { bg = CONTENT_BG }) then
        if gui.selectedSection == 1 then
            drawGeneralSection(ui)
        elseif gui.selectedSection == 2 then
            drawGridSection()
        elseif gui.selectedSection == 3 then
            drawVisualsSection()
        elseif gui.selectedSection == 4 then
            drawBackgroundSection(ui)
        end
    end
...
local GUI_WINDOW_NAME = "Window Utils v2"

--- Initialize section definitions. Call after IconGlyphs is available.
function gui.init()
...
--- @param ui table Reference to the ui module (for blur side-effects)
function gui.drawWindow(ui)
    if not gui.state.showWindow then return end
    ...
    splitter.multi("gui_outer", {
        ...
        -- Bottom: experimental edge toggle (full width, bg matches panels)
...
function gui.show()
    gui.state.showWindow = true
end

function gui.hide()
    gui.state.showWindow = false
end

function gui.toggle()
    gui.state.showWindow = not gui.state.showWindow
end

function gui.isVisible()
    return gui.state.showWindow
end
